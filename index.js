...
const downloadAndExtractMegaZip = async (megaLink, retries = 3, delayMs = 2000) => {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      console.log(`Attempt ${attempt} - Downloading Files from Mega: ${megaLink}`);
      ...
      // Test if the file is accessible by fetching its metadata
      ...
      console.log(`Mega file info: ${JSON.stringify(fileInfo, null, 2)}`);
      ...
      // Verify that critical files (e.g., lib/functions.js) exist after extraction
      ...
      if (attempt === retries) {
        throw new Error(`Failed to download or extract ZIP after ${retries} attempts: ${err.message}`);
      }
...
const downloadResources = async () => {
  try {
    console.log('Fetching PRECIOUS data from GitHub...');
    ...
    console.log('Downloading and extracting files from Mega...');
    await downloadAndExtractMegaZip(zip);
  } catch (error) {
    console.error('Error downloading resources:', error.message);
    throw new Error('Failed to download resources. Cannot proceed without required files.');
  }
};
...
async function connectToWA() {
  ...
  // Download and extract ZIP before proceeding
  await downloadResources();

  // Load dependencies after ZIP extraction
  const l = console.log;
